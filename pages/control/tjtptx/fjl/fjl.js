...
Page({
  ...
  data: {
    ...
    buildArray: ['--请选择--', '地下室工程', '主体工程', '装饰装修工程', '配套工程'],
    categoryArray: [],
    dxsgcArray: ['--请选择--','土方工程','支护工程','基础工程','垫层工程','底板工程','墙柱工程','顶板工程'],
    zszxgcArray: ['--请选择--','砌墙抹灰','门窗安装','扶手栏杆','外墙面层','电梯安装','消防安装','楼低面散'],
    ptgcArray: ['--请选择--','供水工程','供电工程','供气工程','排水排污','道路铺装','绿化工程','亮化工程'],
...
upload1: function(res) {
  ...
  console.log(res.tempFilePaths);
    const uploadTask = wx.uploadFile({
      url: basePath + '/api/tptx/fjl/upload/fileUpload?dir=image',
...
changeDateTimeColumn3(e){
  ...
  bindBuildChange: function (e) {
    ...
    if(buildIndex==1){
      this.setData({
        categoryIndex: 0,
        categoryArray: that.data.dxsgcArray,
      });
    }
    //主体工程
    if(buildIndex==2){
      ...
      wx.request({
        url: basePath + "/api/project/findZtName?pId="+pId, //请求路径
        method: 'post',
        data: {
        },
        header: {
    ...
    if(buildIndex==3){
      this.setData({
        categoryIndex: 0,
        categoryArray: that.data.zszxgcArray,
      });
    }
    if(buildIndex==4){
      this.setData({
        categoryIndex: 0,
        categoryArray: that.data.ptgcArray,
      });
    }
  },
  // 选择具体工程
  bindCategoryChange: function (e) {
    ...
    this.setData({
      ...
      projectName: that.data.categoryArray[e.detail.value],
    });
  },
  ...
  submit: function(e) {
    ...
    wx.request({
      //后台接口
      url: basePath + '/api/tptx/fjl/imgtime/insert',
      method: 'POST',
      data: {
        categoryIndex: that.data.categoryIndex,
        buildIndex: that.data.buildIndex,
        ztName: that.data.projectName,
